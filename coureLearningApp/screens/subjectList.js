...
const subjectList = ({ route, navigation }) => {
    ...
    var itemSubject = [];
    const { role, name, lastname, major, degree, username, docc, idsuj, idTeacher } = route.params;
    ...
    if (role == "student") {
        var set = new Set()
        var docsSubject = []
        keepsubject.filter(doc => { return doc.major == major }).forEach(item => {
            if (!set.has(item.nameSubject)) {
                set.add(item.nameSubject);
                docsSubject.push(item);
            }
        })
        //หลุย มาทำต่อด้วย แต้งกิ้ว
    }
    else if (role == "teacher") {
        var set = new Set()
        var docsSubject = []
        keepsubject.filter(doc => { return doc.idTeacher == idTeacher }).forEach(item => {
            if (!set.has(item.nameSubject)) {
                set.add(item.nameSubject);
                docsSubject.push(item);
            }
    ...
    docsSubject.forEach((doc) => {
        itemSubject.push(<TouchableOpacity style={[styles.row, { padding: 10 }]} onPress={() => {
            ...
            <View style={[styles.col, { fontFamily: "Krub_400Regular" }]}>
                <Text style={[styles.header, { fontFamily: "Krub_400Regular" }]}> {doc.nameSubject}</Text>
                <Text numberOfLines={3} style={[{ color: "white", marginLeft: 5, fontFamily: "Krub_400Regular" }]}>
                    {doc.details}
                </Text>
            </View>
        </TouchableOpacity>)
    ...
    return (
        <View style={styles.fullContainer} >
            ...
            <View style={styles.navTextimg}>
                ...
                <View>
                    <Text style={[styles.headerWelcome, { fontFamily: "Krub_400Regular" }]}>Welcome</Text>
                    <Text style={[styles.headerusername, { fontFamily: "Krub_400Regular" }]}>{username}</Text>
                </View>
                <View>
            ...
            {/* <Dropdown
                ...
                renderLeftIcon={() => (
                    ...
                    /> */}
            <Text style={[styles.subjectHeader, { fontFamily: "Krub_400Regular" }]}>YOUR SUBJECT</Text>
            ...
            <View style={styles.buttonCreate}>
                {role == "student" ?
                    ...
                    <Button title="Add Subject"
                        style={{ fontFamily: "Krub_400Regular" }}
                        color="#937DC2"
                        onPress={() => { navigation.navigate("s2", { idTeacher: idTeacher }) }}
                    />
                }
            </View>
        </View >
