 ...
 const uploadDoc = async () => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={[styles.header, {fontFamily: "Krub_400Regular"}]}>Create Chapter</Text>
      <TextInput
        value={nameChapter} onChangeText={(nameChapter) => { setNameChapter(nameChapter) }}
        style={[styles.textinput, {fontFamily:"Krub_400Regular"}]}
        placeholder="ชื่อบทเรียน"
      />
      <TextInput  multiline={true} value={details} onChangeText={(details) => { setDetails(details) }}

        numberOfLines={10} style={[styles.area, {fontFamily:"Krub_400Regular"}]} placeholder="รายละเอียดวิชา" />

      <TextInput multiline={true} value={video} onChangeText={(video) => { setVideo(video) }} style={[styles.area, {fontFamily:"Krub_400Regular"}]}
        // style={styles.textinput} value={idSubject} onChangeText={(idSubject) => { setIdSubject(idSubject) }}
        placeholder="Link Video"
      />
      <View style={styles.buttonAdd}>
        <Button title="+" style={[styles.add, {fontFamily:"Krub_400Regular"}]} color="#937DC2" onPress={() => pickDoc()} />
        <Text style={[styles.description, {fontFamily:"Krub_400Regular"}]}>อัพโหลดไฟล์</Text>
        <Text style={{fontFamily:"Krub_400Regular"}}>{file}</Text>
        {/* <Button title="upload" style={styles.add} color="#937DC2" onPress={() => uploadDoc()}/> */}
        
      </View>
      <View style={styles.buttonCreate}>
        <Button style={{fontFamily:"Krub_400Regular"}} title="submit"
          onPress={() => { create();  }}
